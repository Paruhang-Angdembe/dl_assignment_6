...
def define_dense_model_with_hidden_layers(
    input_length,
    ...
    for i, (size, activation) in enumerate(
        zip(hidden_layers_sizes, activation_func_array)
    ):
        if i == 0:
            # First hidden layer with input_shape defined
            model.add(
                layers.Dense(size, input_shape=(input_length,), activation=activation)
            )
        else:
